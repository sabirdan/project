...
from PyQt5.QtWidgets import QWidget, QLabel, QPushButton, QFrame, QMessageBox, QHBoxLayout
...
class ControlForm(QWidget):
    def __init__(self, operator_row: dict = None):
        ...
        self.pulse_norm_min, self.pulse_norm_max, self.pulse_crit_threshold = 60, 80, 100
        self.eyes_closed_start_time, self.head_tilted_start_time = None, None
        
        self._load_settings_from_csv()
        self.analysis_form, self.instr_form = None, None

        self.W, self.TITLE_H, self.LINE_H, self.HEADER_H, self.GRID_T = 1000, 30, 4, 120, 4
        ...
        self.current_pulse_val, self.current_state = 0, "NORMAL"
        ...
        self.video_cap, self.current_frame = None, None
        ...
        with open(self.csv_path, "r", newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                if row.get("id") == target_id:
                    ...
                    if "-" in p_norm: self.pulse_norm_min, self.pulse_norm_max = map(int, p_norm.split("-"))
                    elif p_norm.isdigit(): self.pulse_norm_max = int(p_norm)
                    if row.get("pulse_threshold_critical", "").isdigit(): self.pulse_crit_threshold = int(row["pulse_threshold_critical"])
                    break
    ...
    def _build_header(self, col_w):
        ...
        QLabel("Меню управления", menu_frame).setGeometry(0, 15, col_w, 30)
        ...
        btn_instr.setStyleSheet(f"QPushButton {{ background-color: #8D3C7F; {base_style} }} QPushButton:hover {{ background-color: #9E4576; }}")
        ...
        btn_analysis.setStyleSheet(f"QPushButton {{ background-color: #8D3C7F; {base_style} }} QPushButton:hover {{ background-color: #9E4576; }}")
        ...
        btn_control.setStyleSheet(f"QPushButton {{ background-color: #44CC29; {base_style} }}")
        ...
        QLabel("НейроБодр", logo_frame).setGeometry(0, 10, col_w, 50)
        ...
        QLabel("Программа для мониторинга\nсостояния водителей", logo_frame).setGeometry(0, 65, col_w, 50)

        id_frame = QFrame(self)
        id_frame.setGeometry(col_w * 2, self.Y_OFFSET, self.W - col_w * 2, self.HEADER_H)
        
        QLabel("Идентификация", id_frame).setGeometry(0, 5, self.W - col_w * 2, 35)
        
        id_sep = QFrame(id_frame)
        id_sep.setGeometry(0, 45, self.W - col_w * 2, self.GRID_T)
        id_sep.setStyleSheet("background-color: white;")
        
        QLabel("Оператор\nопределен:", id_frame).setGeometry(20, 55, 110, 60)
        
        self.lbl_op_name = QLabel(f"{self.operator_row.get('last_name', '')} {self.operator_row.get('first_name', '')}", id_frame)
        self.lbl_op_name.setGeometry(150, 55, self.W - col_w * 2 - 160, 60)

        for w in [menu_frame, logo_frame, id_frame]:
            for lbl in w.findChildren(QLabel):
                if lbl.text() in ["Меню управления", "НейроБодр"]: lbl.setFont(QFont("Times New Roman", 18 if lbl.text() == "Меню управления" else 20))
                elif lbl.text() == "Идентификация": lbl.setFont(QFont("Times New Roman", 14))
                elif "Программа" in lbl.text(): lbl.setFont(QFont("Times New Roman", 14))
                elif lbl.text() == "Оператор\nопределен:": lbl.setFont(QFont("Times New Roman", 14))
                else: lbl.setFont(QFont("Times New Roman", 16))
                if w == logo_frame: lbl.setStyleSheet("color: white;")
                lbl.setAlignment(Qt.AlignCenter if lbl.text() != "Оператор\nопределен:" and lbl != self.lbl_op_name else Qt.AlignVCenter | Qt.AlignLeft)
    ...
    def _build_left_info_panel(self, parent, w):
        ...
        y = 10
        for title, val, attr in [("Дата/время:", utils._now_date_str() + " / " + utils._now_time_str(), "lbl_dt_val"),
                                 ("Время запуска:", self.operator_row.get("software_start_time", utils._now_time_str()), "lbl_start_val"),
                                 ("Состояние оператора:", "НОРМА", "lbl_state_val")]:
            lbl_t = QLabel(title, parent)
            lbl_t.setGeometry(15, y, 160 if "Состояние" in title else 120, 22)
            lbl_t.setFont(font_label)
            
            lbl_v = QLabel(val, parent)
            lbl_v.setGeometry(180 if "Состояние" in title else 140, y, w - 180, 22)
            lbl_v.setFont(font_val)
            if "Состояние" in title: lbl_v.setStyleSheet("color: #009900;")
    ...
    def _process_camera_frame(self):
        ...
        detected_eyes, detected_face = False, False

        if len(faces) > 0:
            detected_face = True
            x, y, w, h = sorted(faces, key=lambda f: f[2]*f[3], reverse=True)[0]
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
            eyes = self.eye_cascade.detectMultiScale(gray[y:y+h, x:x+w], 1.1, 10, minSize=(20, 20))
            if len(eyes) > 0:
                detected_eyes = True
                for ex, ey, ew, eh in eyes: cv2.rectangle(frame[y:y+h, x:x+w], (ex, ey), (ex+ew, ey+eh), (255, 0, 0), 2)

        curr_time = time.time()
        if detected_face and not detected_eyes:
            if not self.eyes_closed_start_time: self.eyes_closed_start_time = curr_time
        ...
        if not detected_face:
            if not self.head_tilted_start_time: self.head_tilted_start_time = curr_time
        else:
    ...
    def _check_status(self):
        curr_time = time.time()
        sec_closed = (curr_time - self.eyes_closed_start_time) if self.eyes_closed_start_time else 0
        sec_tilted = (curr_time - self.head_tilted_start_time) if self.head_tilted_start_time else 0
        
        p = self.current_pulse_val
        new_state = "NORMAL"

        if (p > 0 and (p <= self.pulse_norm_min * 0.7 or p >= self.pulse_crit_threshold)) or sec_closed > 7.0 or sec_tilted > 7.0:
            new_state = "CRITICAL"
        elif (p > 0 and (p <= self.pulse_norm_min * 0.8 or (p >= self.pulse_norm_max * 1.2 and p < self.pulse_crit_threshold))) or sec_closed > 4.0 or sec_tilted > 4.0:
            new_state = "WARNING"
        ...
        if self.current_state == "NORMAL": self.lbl_term_text.setText(f"Состояние нормальное\nПульс {p_str}")
        elif self.current_state == "WARNING": self.lbl_term_text.setText(f"Состояние оператора выходит за пределы «ВНИМАНИЕ»\nПульс {p_str}\nЗапуск звукового оповещения «ВНИМАНИЕ»")
        else: self.lbl_term_text.setText(f"Состояние критичное!\nПульс {p_str}\nЗапуск звукового оповещения!")
    ...
    def _update_time_logic(self):
        ...
        if self.remaining_seconds > 0:
            ...
            self.lbl_clock.setText(f"{self.remaining_seconds // 3600:02d}:{(self.remaining_seconds % 3600) // 60:02d}")
        else:
    ...
    def _update_csv_log(self):
        ...
        updates = {"current_pulse": str(self.current_pulse_val), "operator_status": self.current_state, "drive_duration": utils._seconds_to_hms((datetime.datetime.now() - self.start_app_time).total_seconds())}
        try:
            with open(self.csv_path, 'r', newline='', encoding='utf-8') as f:
    ...
    def _go_instruction(self):
        ...
        if not self.instr_form: self.instr_form = InstructionForm(self.operator_row)
        self.instr_form.show()

    def _go_analysis(self):
        self.close()
        from analysis_form import AnalysisForm
        if not self.analysis_form: self.analysis_form = AnalysisForm(self.operator_row)
        self.analysis_form.show()

    def closeEvent(self, event):
        self._update_csv_log()
        if hasattr(self, 'worker_pulse'): self.worker_pulse.stop()
        if hasattr(self, 'thread_pulse'):
            self.thread_pulse.quit()
            self.thread_pulse.wait()
        for t in [self.timer_main, self.timer_cam, self.timer_video]: t.stop()
