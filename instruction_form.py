...
class ShapeWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.shape_type == "circle":
            ...
            painter.drawPolygon(QPolygon([QPoint(self.width() // 2, 0), QPoint(0, self.height()), QPoint(self.width(), self.height())]))
...
class InstructionForm(QWidget):
    def __init__(self, operator_row: dict = None):
        ...
        self.analysis_form, self.control_form = None, None
    ...
    def _build_ui(self):
        col_one_w = self.W // 3
        left_body_w, right_body_w = col_one_w * 2, self.W - col_one_w * 2
        ...
        base_style = "QPushButton { color: white; border-radius: 18px; font-family: 'Times New Roman'; font-size: 14px; font-weight: bold; }"

        self.btn_instr = QPushButton("Инструкция", menu_frame)
        self.btn_instr.setGeometry(0, btn_y, btn_w, btn_h)
        self.btn_instr.setStyleSheet(base_style + "QPushButton { background-color: #44CC29; } QPushButton:hover { background-color: #45D44A; }")

        self.btn_analysis = QPushButton("Анализ", menu_frame)
        self.btn_analysis.setGeometry(btn_w + spacing, btn_y, btn_w, btn_h)
        self.btn_analysis.setStyleSheet(base_style + "QPushButton { background-color: #8D3C7F; } QPushButton:hover { background-color: #9E4576; }")
        self.btn_analysis.clicked.connect(self._open_analysis)

        self.btn_control = QPushButton("Управление", menu_frame)
        self.btn_control.setGeometry((btn_w + spacing) * 2, btn_y, col_one_w - (btn_w * 2 + spacing * 2), btn_h)
        self.btn_control.setStyleSheet(base_style + "QPushButton { background-color: #8D3C7F; } QPushButton:hover { background-color: #9E4576; }")
    ...
    def _build_connection_content(self, parent, x_offset, w):
        ...
        for i, img in enumerate(["hand1.png", "hand2.png"]):
            ...
            box.setPixmap(QPixmap(f"assets/{img}").scaled(block_w, block_h, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        btn_next.setStyleSheet("QPushButton { background-color: #2C2C2C; color: #FFFFFF; border-radius: 6px; font-weight: bold; font-family: 'Times New Roman'; font-size: 14px; } QPushButton:hover { background-color: #44CC29; }")
