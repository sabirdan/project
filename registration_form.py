...
from PyQt5.QtWidgets import QLineEdit, QFrame, QMessageBox, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout
...
class RegistrationForm(QWidget):
    def __init__(self, start_screen, csv_file: str, ops_dir: str, software_start_time: str):
        ...
        self.start_screen, self.csv_file, self.ops_dir, self.software_start_time = start_screen, csv_file, ops_dir, software_start_time

        self.current_id, self.cap, self._known_enc_cache, self.last_frame = None, None, None, None
    ...
    def reset_form(self):
        self.current_id, self._known_enc_cache, self.last_frame = None, None, None
        for inp in (self.in_last, self.in_first, self.in_middle, self.in_age): inp.clear()
        
        self._set_status(False, assigned=False)
        self.btn_next.setEnabled(False)
        self.btn_identify.setEnabled(False)
        self.cam_view.setPixmap(QPixmap("assets/face.png").scaled(self.cam_view.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def _build_ui(self):
        ...
        self.btn_identify.setGeometry((col_w - 200) // 2, (self.SECTION_H - 35) // 2, 200, 35)
        self.btn_identify.setStyleSheet("""
            QPushButton { background-color: #2C2C2C; color: #FFFFFF; border-radius: 6px; font-size: 16px; }
        ...
        y0, field_step = self.SECTION_H + 30, 48
        ...
        self.btn_save = self._small_btn("Записать", self.left, col_w - 138, btn_y_level, self._on_save)
        self.btn_next = self._small_btn("Далее", self.right, col3_w - 130, btn_y_level, self._go_start)

        cam_w, cam_h = col_w - 25, 220
    ...
    def _small_btn(self, text, parent, x, y, callback):
        ...
        btn.setGeometry(x, y, 120 if text == "Записать" else 100, 34)
        btn.setCursor(Qt.PointingHandCursor)
        btn.setStyleSheet("""
            QPushButton { background-color: #2C2C2C; color: #FFFFFF; border-radius: 6px; font-weight: 600; }
            QPushButton:hover { background-color: #44CC29; }
            QPushButton:disabled { background-color: #BDBDBD; color: #6B6B6B; }
        """)
    ...
    def _set_status(self, ok: bool, assigned: bool):
        ...
        self.id_banner.setText(f"ID {_id_str(self.current_id)}" if assigned and self.current_id else "ID не присвоен")

    def _validate_fields(self):
        ln, fn, mn, age = self.in_last.text().strip(), self.in_first.text().strip(), self.in_middle.text().strip(), self.in_age.text().strip()
        name_re = re.compile(r"^[A-Za-zА-Яа-яЁё \-]{1,50}$")
        
        if not ln or not fn: return None, "Фамилия и Имя обязательны."
        if not name_re.match(ln) or not name_re.match(fn) or (mn and not name_re.match(mn)):
            return None, "ФИО: только буквы, пробел, дефис."
        if not age.isdigit() or int(age) < 18: 
            return None, "Возраст должен быть числом не менее 18 лет."
            
        return {"last_name": ln, "first_name": fn, "middle_name": mn, "age": age}, None

    def _append_csv_row(self, operator):
        new_id = _next_id(self.csv_file)
        with open(self.csv_file, "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow([
                str(new_id), _safe_csv_cell(operator["last_name"]), _safe_csv_cell(operator["first_name"]),
                _safe_csv_cell(operator["middle_name"]), operator["age"], _now_date_str(),
                _now_time_str(), self.software_start_time, "00:00:00"
            ])
        return new_id
    ...
    def _try_verify(self):
        ...
        if not self._known_enc_cache: self._known_enc_cache = cv_load_known_faces(self.ops_dir, exclude_id=self.current_id)

        if live_face_gray is None or cv_find_match(self._known_enc_cache, live_face_gray) is not None:
            self._set_status(False, assigned=True)
        ...
        _opencv_save_jpg(self.last_frame, os.path.join(self.ops_dir, f"ID_{_id_str(self.current_id)}.jpg"), face_loc=live_loc)
